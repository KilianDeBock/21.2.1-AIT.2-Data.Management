...
$used_verbs = $verbs;
if (count($config) > 0) {
    $used_verbs = array_filter($verbs, function ($verb) use ($config) {
        return in_array($verb, $config);
    }, ARRAY_FILTER_USE_KEY);
}
if (isset($_POST['config'])) {
    $config = $_POST['config'];
}
setcookie('config', json_encode($config), time() + 3600);
...
$next_verb = array_rand($used_verbs);
